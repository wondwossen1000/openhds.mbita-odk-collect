...
public class FormEntryActivity extends Activity implements AnimationListener, FormLoaderListener,
    ...
    public static final int ENTITY_ID_ACTIVITY = 10;
    ...
    private static final int MENU_LOOKUP_IDS = Menu.FIRST + 4;
        ...
        switch (requestCode) {
            ...
            case ENTITY_ID_ACTIVITY:
           	...
           	if (qw instanceof StringWidget) {
           	   	StringWidget widget = (StringWidget)qw;
               	widget.setAnswer(intent.getAction());
               	saveAnswer(qw.getAnswer(), mFormController.getFormIndex(),false);
           	}
           	 }
           	break;
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (mFormController.getFormIndex().getLocalIndex() >= 0) {
        	 for (QuestionWidget qw : ((ODKView) mCurrentView).getWidgets()) {
	        menu.add(0, MENU_LOOKUP_IDS, 0, getString(R.string.lookupIds))
	        	.setIcon(android.R.drawable.ic_menu_search)
	        	.setEnabled(qw instanceof StringWidget ? true : false);
        	 }
        }
    ...
    /*public int saveAnswer(IAnswerData answer, boolean evaluateConstraints) {
        if (evaluateConstraints) {
            return mFormController.answerQuestion(answer);
        } else {
        	mFormController.saveAnswer(mFormController.getFormIndex(), answer);
            return FormEntryController.ANSWER_OK;
        }
    }*/
